...
\else
  \chapter{Results}
    ...
    Figure [X] illustrates the baseline accuracy of the student model. These results serve as a primary benchmark for comparison against the distilled model, which is trained using the soft outputs of the teacher model.

    \includegraphics{img/student_baseline_v1.png}
    ...
    In summary, this research successfully established a pipeline for model compression. However, several limitations must be acknowledged. First, the reliance on a fixed, balanced dataset, while providing a controlled environment for initial testing, may mask the model's performance in real-world, imbalanced scenarios.
    ...
    [Final summary table comparing models' accuracy, size, latency]
